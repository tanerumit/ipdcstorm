...
This work presents a parametric wind hazard model developed for the Dutch Caribbean region to support climate stress testing of the supply chain of the Statia and Saba islands.

The main purpose of the model is to produce a range of climate scenarios that characterize the probability and intensity of hurricane wind exposure at point-locations representing the islands.

The model combines NOAA's historical storm database (IBTrACS) with stochastic simulation to generate long synthetic records of daily wind forcing suitable as input to a system dynamics model.

The modeling scheme consists of three workflows:

1.  **Historical calibration**\
    
The first workflow extracts observed tropical cyclone tracks from the IBTrACS North Atlantic database and computes site-level wind speeds for each historical storm using a Holland (1980) parametric wind profile with forward-motion asymmetry.  
    
The wind profile uses directional wind radii from IBTrACS (where available) and climatological fallbacks to estimate storm size at each target location.  
    
From the resulting site-specific event catalog, the model estimates storm arrival rates by severity class (tropical storm vs. hurricane) and a Gamma overdispersion parameter that captures the year-to-year clustering of active and quiet seasons.

2.  **Stochastic simulation**\
    
The simulation workflow takes the calibrated rates and overdispersion parameter to generate a long series of synthetic hurricane seasons.  
  
Each simulated year draws a random activity factor from a Gamma distribution to reproduce the observed burstiness of seasons, then draws storm counts from a Poisson distribution conditional on that activity level. 

The total count is split into tropical storms and hurricanes via a binomial draw.  

Climate change enters through two SST-driven mechanisms: a rate effect that scales overall annual activity, and an intensity effect that shifts the storm-class split toward a higher hurricane fraction under warming.

An optional third level perturbs individual storm properties (peak intensity, size, translation speed) for scenario exploration.

3.  **Temporal downscaling**\
    
The final workflow converts simulated annual storm counts into a daily wind series for each island. For each storm in a simulated year, a historical event is resampled — with its physical attributes — from a severity-stratified library built during calibration.  
    
Arrival dates are drawn from empirical day-of-year distributions that preserve the observed seasonality (the August–October peak).Each event is spread over its duration using a smooth rise-and-fall wind envelope that peaks at the sampled storm intensity, and overlapping events are combined into a 365-day time series of daily wind speeds (kt).  
    
The model also flags supply-chain disruption (port closure, infrastructure damage) by comparing daily winds against configurable thresholds.  
    
Combining all simulated years yields a long stochastic daily wind record that serves as direct forcing input for the system dynamics model.
...
cfg <- make_hazard_cfg(
  ...
  historical_start_year = 1970L,  # use data from 1970 onward (satellite era)
  search_radius_km      = 800,    # only consider storms passing within 800 km
  simulation_years      = 1000L   # number of years to simulate
)
...
| `historical_start_year` | Start of the calibration period. 1970 marks the beginning of reliable satellite-era observations in the Atlantic. |
...
| `simulation_years` | Number of synthetic years to generate (default = 1,000 years). |
...
With the default climate configuration, the model runs in **stationary mode** (no climate change):
...
| `storm_class` | `TS` (tropical storm, 34–63 kt) or `HUR` (hurricane, ≥64 kt) |
...
| `storm_class` | Classification based on site-level wind: `TS` or `HUR` |
| `location` | Target site name |
...
To get a **daily time series**, use `generate_daily_hazard_impact_spatial()`:
...
**`simulation_years`: ** (simulation length). This sets how many synthetic years you generate. More years reduce Monte Carlo noise but increase runtime.

**`search_radius_km`: ** (track-point gate radius). This sets how far from the site a storm track point can be and still be considered. The default 800 km is conservative and usually captures all relevant storms without pulling in clearly irrelevant ones.

**`gust_factor`: ** (sustained-to-gust conversion). The gust_factor multiplies sustained wind to estimate gust wind for comparisons and downstream rules. Typical values are 1.2–1.3 for open terrain, and up to 1.4 for complex terrain or urban exposure.
