...
This tutorial scales the workflow from a single site to a small group of
islands, then compares a present-day baseline with a warmer climate. The
aim is practical: run the model, inspect the annual output, and turn
scenario differences into daily disruption metrics for one site.
...
- turn the annual scenario differences into day-by-day disruption
...
We start with the three Dutch Caribbean islands used throughout the
package: St. Martin, Saba, and Statia. The setup is the same as in
Tutorial 1, only with three rows in `targets` instead of one.
...
The `rates` table shows what the model learned from history: the average
number of tropical-storm and hurricane hits per year at each island.
...
The simulation table has one synthetic season per row. To compare
islands at a glance, average the simulated counts and the hurricane
share by location.
...
These numbers should broadly match the `rates` table. Small differences
between islands reflect real geography: some sites see storms pass close
and strong more often than others.

## 3. Climate Change Scenarios

Warming affects the simulated climate in two ways. First, it can raise
or lower the total number of storms per year (a *rate* effect). Second,
it can shift the storm mix so a larger share reach hurricane strength
(an *intensity* effect).

Both effects are driven by a single number: `delta_sst`, the change in
Main Development Region sea-surface temperature (in °C) relative to a
1991-2020 baseline. You can either use a named scenario helper (IPCC
AR6, KNMI’23) or set `delta_sst` directly.

Here we compare a stationary baseline (`delta_sst = 0`) with a warmer
climate at `delta_sst = 1.5`. This side-by-side setup makes the
direction and size of the climate response easy to see.
...
The built-in scenario table is useful context, but for the tutorial we
set `delta_sst` directly so the warming is visible in the code.
...
The summary reports the average yearly storm count, how it splits into
tropical storms and hurricanes, and the simulated hurricane share.
...
>
> Scenario differences describe the long-run average, not a forecast for
> any single year. A higher `mean_total` or `mean_p_hur` means the
> simulated climate is more active on average — but quiet years still
> happen.

## 4. Daily Series Comparison

Annual counts are useful for climatology, but many planning questions
are about daily disruption. To show that step, we generate daily hazard
and impact series for Saba under the baseline and the warmer scenario.
...
The combined table shows how often Saba experiences tropical-storm and
hurricane days under each scenario, plus the average annual damage
signal across the 200 simulated years.
...
> **Adding trade-wind background to both scenarios**
>
> The daily series above have zero wind on every non-storm day. For
> applications that need a continuous wind record (wind energy, chronic
> exposure, port disruption), attach a
> [`make_background_wind_cfg()`](https://tanerumit.github.io/ipdcstorm/reference/make_background_wind_cfg.md)
> object to each hazard config. It then flows through to
...
> call
> [`generate_daily_hazard_impact_spatial()`](https://tanerumit.github.io/ipdcstorm/reference/generate_daily_hazard_impact_spatial.md)
> without a `background_wind` argument — it picks the setting up from
> `out$cfg$background_wind` automatically, keeping the same seed so the
> climate comparison stays clean. See [Tutorial 1 §Background
> Wind](https://tanerumit.github.io/ipdcstorm/articles/tutorial_1_setup.qmd#sec-background-wind)
> for guidance and a worked example.

> **About event-level perturbation**
>
> The package also supports event-level perturbation for more
> exploratory scenario analysis. We do not use it here, so the daily
> comparison isolates the rate and intensity effects discussed above.
...
Baseline-versus-scenario is a good starting point for stress-testing.
Once the annual and daily outputs feel familiar, the same workflow
scales to more islands or alternative warming assumptions.
