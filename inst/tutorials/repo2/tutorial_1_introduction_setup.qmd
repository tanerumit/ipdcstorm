...
1.  **Historical calibration**\
    ...
    The wind profile uses directional wind radii from IBTrACS (where available) and climatological fallbacks to estimate the spatial extent of each storm's wind field at each target location.
    From the resulting site-specific event catalog, the model estimates long-term storm arrival rates by severity class (tropical storm vs. hurricane), along with a Gamma overdispersion parameter that captures the observed year-to-year clustering of active and quiet hurricane seasons.

2.  **Stochastic simulation**\
    The simulation workflow takes the calibrated rates, overdispersion parameter, and a climate forcing specification to generate a long series of synthetic hurricane seasons.
    Each simulated year first draws a random activity factor from a Gamma distribution to reproduce the observed burstiness of hurricane seasons, then draws total storm counts from a Poisson distribution conditional on that activity level.
    The total count is split into tropical storms and hurricanes using a binomial draw.
    Climate change enters through two SST-driven mechanisms: a rate effect that scales the overall annual activity (via an exponential SST sensitivity coefficient), and an intensity effect that shifts the tropical storm / hurricane split toward a higher hurricane fraction under warming. An optional third level perturbs individual storm properties (peak intensity, size, translation speed) as a function of SST change for scenario exploration.

3.  **Temporal downscaling**\
    The final workflow converts the simulated annual storm counts into a daily wind series for each island.For each storm in a simulated year, a historical event is resampled (with its physical attributes) from a severity-stratified event library built from the calibrated record.
    Storm arrival dates are drawn from empirical day-of-year distributions that preserve the observed seasonality of hurricane landfalls (i.e., the August–October peak).
    Each event is then spread over its duration using a smooth rise-and-fall wind envelope (cosine shape) that peaks at the sampled storm intensity. Overlapping events are combined into a single 365-day time series of daily peak sustained wind speeds (kt), with the dominant event tracked for each day. The model also flags site-specific supply-chain disruption (port closure, infrastructure damage) by comparing daily winds against configurable thresholds.
    Combining all simulated years yields a long stochastic daily wind and disruption record that serves as direct forcing input for the system dynamics model of supply-chain vulnerability.
