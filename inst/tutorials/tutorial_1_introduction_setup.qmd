...
## Single-Site Baseline {#sec-single-site}

In this section we run the model for a **single site** with no climate change — the **stationary baseline**. By the end you will have worked through all three workflows and produced:
...
- **Daily wind time series**: A day-by-day record of wind exposure with damage rates


### Step 1 — Create the configuration {#sec-cfg}

Use `make_hazard_cfg()` to create a configuration object. This is the entry point for all model settings — it validates inputs and provides sensible defaults so that most users only need to set the data path and simulation length:
...
|-----------|-------------|
| `data_path` | Path to the IBTrACS North Atlantic CSV file. |
| `start_year` | Start of the calibration period. 1970 marks the beginning of reliable satellite-era observations in the Atlantic. Earlier data has known intensity biases. |
| `search_radius_km` | The model only processes IBTrACS track points within this radius of the target site. This is a computational filter — not a physical threshold. A storm 700 km away can still produce tropical storm winds at your site through its wind field extent. |
| `n_sim_years` | Number of synthetic years to generate. 1,000 is a good default — enough for stable statistics up to roughly the 200-year return period. |

Wind speed thresholds and wind radii caps use standard WMO values by default (TS ≥ 34 kt, Hurricane ≥ 64 kt). These are set automatically by the `"default"` preset and should not need changing for most applications.
:::

::: {.callout-tip}
## Advanced tuning
If you do need to override thresholds or caps, pass them via the `advanced` argument:
...
Only specify the parameters you want to change — the rest keep their defaults. See `?make_hazard_cfg` for the full list of supported names.
:::
...
### Workflow 1 & 2 — Historical calibration and stochastic simulation {#sec-run}

Both the historical calibration (Workflow 1) and stochastic simulation (Workflow 2) are executed together by `run_hazard_model()`. With `sst_cfg = NULL`, the model runs in **stationary mode** (no climate change):
...
The function executes six steps for each target location. Steps 1–5 correspond to Workflow 1 (historical calibration); Step 6 corresponds to Workflow 2 (stochastic simulation).
...
For each target, the model computes the great-circle distance from every IBTrACS track point to the site. Only points within `search_radius_km` (800 km) are retained. This reduces the dataset from hundreds of thousands of track points to the subset that is physically close enough to potentially produce measurable winds at the site.
...
The result is a `site_wind_kt` value — estimated sustained wind in knots at the site — for every 6-hourly track point.

**Step 4 — Aggregate into storm events.**
Track points are grouped by storm ID (`storm_id`) into event-level summaries via `make_storm_events()`. Each event record captures the **peak site wind** (`peak_wind_kt`), the storm's overall maximum intensity, minimum central pressure, mean RMW, start/end times, and the number of track points within the gate. Events are classified by storm class:

  - **Tropical storm** (`TS`): peak site wind 34–63 kt
  - **Hurricane** (`HUR64plus`): peak site wind ≥64 kt

Events from before `start_year` (1970) and those with unknown site winds are excluded.

**Step 5 — Estimate annual rates.**
Historical events are counted by year and storm class (`compute_annual_counts()`, `compute_lambda_table()`). The result is a Poisson rate $\lambda$ for each storm class — for example, $\lambda_{\text{TS}} = 1.2$ means on average 1.2 tropical storm events per year produce ≥34 kt winds at this site.
...
Using the estimated $\lambda$ values and $k$, the model generates `n_sim_years` synthetic hurricane seasons via `simulate_twolevel_counts()`. For each simulated year:

  1. An activity factor is drawn: $A \sim \text{Gamma}(k, k)$ — mean 1, variance $1/k$
  2. Total storms are drawn: $N_{\text{tc}} \sim \text{Poisson}(A \cdot \lambda_{\text{total}})$
  3. Each storm is classified: $N_{\text{hur}} \sim \text{Binomial}(N_{\text{tc}}, p_{\text{hur}})$

This produces a table of 1,000 rows — one per simulated year — each with storm counts by class.


### Step 3 — Inspect the outputs {#sec-outputs}
...
The distinction between `peak_wind_kt` and `storm_intensity_kt` is important: a Category 5 storm (155 kt peak) passing 400 km from your site might only produce Category 1 winds (70 kt) there. The model captures this attenuation through the Holland wind profile.
...
The lowest-level output: every 6-hourly IBTrACS track point within the search radius, with all computed wind field columns (`site_wind_kt`, `dist_km`, `bearing_deg`, `RMW_km`, etc.). Useful for diagnostics and spot-checking individual storms.
...
| `scenario` | Label for the climate scenario (for downstream bookkeeping) |
| `date` | Calendar date |
...
| `surge_m` | Estimated storm surge (m), derived from central pressure via the inverted-barometer approximation |
| `event_id` | Identifier of the dominant storm event on this day (NA on calm days) |
| `event_class` | Storm class of the dominant event (`TS` or `HUR`) |
| `pressure_hpa` | Central pressure of the dominant event |
| `pressure_deficit_hpa` | Pressure deficit of the dominant event |
| `rmw_km` | RMW of the dominant event |
| `damage_intensity` | Normalised hazard intensity (0–1 scale) |
...
The daily output provides `wind_gust_kt` which you can threshold downstream for port closures or infrastructure disruption. For example, to flag port disruption at 40 kt gusts: `daily |> mutate(port_disrupt = wind_gust_kt >= 40)`.
:::
...
### Search radius (`search_radius_km`)
...
The `damage_method = "intensity"` option uses a simpler normalised intensity curve capped at `dmax`.
...
| Function | Purpose |
|----------|---------|
| `make_hazard_cfg()` | Create a validated configuration object with sensible defaults |
...
| `make_sst_cfg()` | Configure climate change scenarios (Tutorial 2) |
...
- **Tutorial 1** (this document) — Introduction, setup, and single-site baseline (all three workflows)
- **Tutorial 2** — Multi-site application, model validation, and climate change scenarios
:::
