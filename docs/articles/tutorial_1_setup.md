...
We introduce the model, set up the configuration, fit the stationary
baseline, and generate the daily wind series for three Dutch Caribbean
targets.
...
Saba, Statia) but works for any point location in the North Atlantic
basin.
...
>
> This tutorial covers the stationary baseline. [Tutorial
> 2](https://tanerumit.github.io/ipdcstorm/articles/tutorial_2_validation.qmd)
> checks model skill via the three-tier validation suite. [Tutorial
> 3](https://tanerumit.github.io/ipdcstorm/articles/tutorial_3_climate_change.qmd)
> layers KNMI’23 and IPCC AR6 climate-change scenarios on top of the
> same workflow.
...
Stewardship (IBTrACS)**, the most comprehensive global dataset of
tropical cyclone tracks. The model uses the **North Atlantic subset**
...
For this tutorial we use the packaged 1970+ subset so it renders
end-to-end.
...
`inst/extcode/01-baseline-analysis.R`. Lower `N_SIM` while exploring;
use ≥ 1,000 for any defensible result and 2,000+ for production.
...
## 4 Stages 1 & 2 — Calibration and Simulation

[`run_hazard_model()`](https://tanerumit.github.io/ipdcstorm/reference/run_hazard_model.md)
executes the historical calibration (Stage 1) and stochastic simulation
...
Standardise columns, retain the USA best-track wind/pressure fields,
...
Determine the site’s quadrant relative to the storm centre, pull the
...
> site’s `lambda` is far from a published HURDAT2 rate, the rest of the
> workflow will inherit that bias — fix it before downstream analysis.
> Tutorial 2’s Tier 2 check formalises this.
...
## 5 Stage 3 — Daily Wind Time Series
...
**Step 6 — Derived daily fields.** Apply the gust factor, the damage
function (`damage = list(method = "intensity")` returns the normalized
intensity-based damage proxy; `"powerlaw"` is also available), and
forward dominant-event metadata (`event_id`, `event_class`,
`pressure_hpa`, `rmw_km`).
...
> climate forcing. This is the pattern used in Tutorial 3 and the
> stress-test experiment script.

> **Pinned events**
>
> `pinned_sids` lets you force a specific historical SID into one or
> more sampled event slots — used by the stress-test workflow in
> Tutorial 4 to keep a focal storm (e.g. Irma) present across every
> scenario year.
...
```
    ...
    #> # A tibble: 6 × 15
    #>   location sim_year scenario   date       wind_kt wind_gust_kt surge_m event_id
    #>   <chr>       <int> <chr>      <date>       <dbl>        <dbl>   <dbl> <chr>   
    #> 1 Saba            1 stationary 2000-01-01       0            0      NA <NA>    
    #> 2 Saba            1 stationary 2000-01-02       0            0      NA <NA>    
    #> 3 Saba            1 stationary 2000-01-03       0            0      NA <NA>    
    #> 4 Saba            1 stationary 2000-01-04       0            0      NA <NA>    
    #> 5 Saba            1 stationary 2000-01-05       0            0      NA <NA>    
    #> 6 Saba            1 stationary 2000-01-06       0            0      NA <NA>    
    #> # ℹ 7 more variables: event_class <chr>, pressure_hpa <dbl>,
    #> #   pressure_deficit_hpa <dbl>, rmw_km <dbl>, damage_intensity <dbl>,
    #> #   damage_rate <dbl>, cum_damage <dbl>
...
need a realistic non-storm wind envelope — energy yield modelling,
chronic-exposure assessments, port disruption studies.
...
Weibull distribution; the storm-day wind from the TC pulse then replaces
it via [`pmax()`](https://rdrr.io/r/base/Extremes.html), so the TC
signal always dominates on active days.

#### 5.3.1 Configuring background wind
...
The `ar1` coefficient controls how strongly today’s background wind
predicts tomorrow’s in standardised normal space. `ar1 = 0` gives
independent daily draws; `ar1 = 0.4` introduces realistic multi-day
persistence similar to synoptic trade-wind patterns. For multi-site
runs, `cor_matrix` lets you specify site-to-site Pearson correlations so
that calm or windy spells are spatially coherent.

#### 5.3.2 Embedding in the hazard configuration (recommended)

The cleanest production pattern attaches the background wind
configuration to
[`make_hazard_cfg()`](https://tanerumit.github.io/ipdcstorm/reference/make_hazard_cfg.md).
It is then stored in `out$cfg` and picked up automatically by
[`generate_daily_hazard_impact_spatial()`](https://tanerumit.github.io/ipdcstorm/reference/generate_daily_hazard_impact_spatial.md)
...
#### 5.3.3 Passing background wind directly (exploratory use)

If you already have a model output object and want to try different
background configurations without re-running the full calibration, pass
`background_wind` directly to
[`generate_daily_hazard_impact_spatial()`](https://tanerumit.github.io/ipdcstorm/reference/generate_daily_hazard_impact_spatial.md).
...
#### 5.3.4 Effect on the daily series

Comparing 50 simulated years for Saba shows the reduction in zero-wind
days and the resulting uplift in non-storm-period mean wind:
...
```
    ...
    #> # A tibble: 2 × 4
    #>   run             pct_zero_wind mean_wind_all_kt mean_wind_nonzero
    #>   <chr>                   <dbl>            <dbl>             <dbl>
    #> 1 No background            97.9             0.69              32.2
    #> 2 With background           0              11.9               11.9

`pct_zero_wind` drops to near zero once background wind is active.
`mean_wind_all_kt` rises to reflect the trade-wind baseline;
`mean_wind_nonzero` stays close to the TC-period mean because those days
are dominated by the storm pulse, not the background.
...
- [Tutorial 2 — Model
  Validation](https://tanerumit.github.io/ipdcstorm/articles/tutorial_2_validation.qmd)
  runs the three-tier validation suite (hindcast return levels, rate
  sanity, wind-field spot-checks) on this same `out_base`.
- [Tutorial 3 — Climate
  Change](https://tanerumit.github.io/ipdcstorm/articles/tutorial_3_climate_change.qmd)
  replaces the stationary `climate_cfg` with KNMI’23 and IPCC AR6
  scenarios and shows how to compare baseline vs scenario daily series
  with common random numbers.
- The stress-test workflow in `inst/extcode/05-stress-test-experiment.R`
  selects representative high-impact years from the baseline and replays
  them under climate scenarios with a pinned focal event.
