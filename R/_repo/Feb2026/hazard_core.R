  ...
  # - classify_severity(): severity class from peak site wind.
  # - make_storm_events(): aggregate track points into storm-level events.
  # - compute_annual_counts(): annual event counts by severity.
  # - compute_lambda_table(): Poisson rate summaries by severity.
  # - estimate_k_hat(): overdispersion estimate for annual totals.
  # - simulate_twolevel_counts(): two-level Poisson-Gamma annual counts.
  ...
  estimate_R34_climo <- function(Vmax_kt, lat = 18) {
    ...
    #   R34_nm ≈ a + b·(Vmax-34) + c·(Vmax-34)^2 + d·|lat-25|
    # Coefficients estimated from Fig 4/Table 3 of Knaff et al. 2015
    a <- 68    # base R34 at TS threshold (nm) — Atlantic mean
  ...
  #' the key relationship: intense low-latitude storms (like Irma at 17°N) have
  ...
  estimate_RMW_knaff <- function(Vmax_kt, lat = 18) {
    ...
    #   RMW_nm = 66.785 - 0.09102·Vmax + 1.0619·(lat - 25)
  ...
  .estimate_site_wind_holland_v2 <- function(
    ...
    if (any(can_cal)) {
      ...
      if (any(good)) {
        ...
        # Blend if climatological R34
        is_climo_u <- R34_is_climo[use][can_cal]
        blend <- is_climo_u & good
        if (any(blend)) {
          cal_factor[blend] <- 1 + 0.7 * (cal_factor[blend] - 1)
        }
  ...
  .add_forward_motion_asymmetry <- function(
    ...
    if (any(i1)) K[i1] <- 0.4 + 0.25 * rn[i1]
    if (any(i2)) K[i2] <- 0.65 - 0.15 * (rn[i2] - 1.0) / 2.0
    if (any(i3)) K[i3] <- 0.50 * exp(-0.2 * (rn[i3] - 3.0))
  ...
  compute_site_winds_full <- function(df, target_lat, target_lon) {
    ...
    # --- Radii infill (minimal): if R34 missing but storm is TS+, use a climatological default
    # Choose conservative defaults (km) so you don't blow up winds everywhere.
    # df <- df |>
    #   dplyr::mutate(
    #     R34_km = dplyr::case_when(
    #       is.finite(.data$R34_km) ~ .data$R34_km,
    #       is.finite(.data$Vmax_kt) & .data$Vmax_kt >= 34 & .data$Vmax_kt < 64 ~ 150,  # TS typical order
    ...
    #       TRUE ~ NA_real_
    #     ),
    #     R64_km = dplyr::case_when(
    #       is.finite(.data$R64_km) ~ .data$R64_km,
    #       is.finite(.data$Vmax_kt) & .data$Vmax_kt >= 64 & is.finite(.data$R34_km) ~ 0.35 * .data$R34_km,
    #       TRUE ~ NA_real_
    #     )
    #   )
    # 
...
#' Classify storm severity from peak site wind
#'
...
#' @export
  ...
  #'
  #' @return Tibble with one row per SID and key event attributes.
  #' @export
  make_storm_events <- function(track_df) {
    ...
    out <- df |>
      ...
      dplyr::summarise(
        ...
        any_Vsite_known = any(is.finite(.data$V_site_kt)),
        V_site_max_kt   = suppressWarnings(max(.data$V_site_kt, na.rm = TRUE)),
        
        wind_max_kt = suppressWarnings(max(.data$wind_kt, na.rm = TRUE)),
        
        Pc_min_hPa  = suppressWarnings(min(.data$pres_hpa, na.rm = TRUE)),
        dP_max_hPa  = suppressWarnings(max(.data$dP_hpa, na.rm = TRUE)),
      ...
      dplyr::mutate(
        V_site_max_kt = dplyr::if_else(is.finite(.data$V_site_max_kt), .data$V_site_max_kt, NA_real_),
        wind_max_kt   = dplyr::if_else(is.finite(.data$wind_max_kt),   .data$wind_max_kt,   NA_real_),
        Pc_min_hPa    = dplyr::if_else(is.finite(.data$Pc_min_hPa),    .data$Pc_min_hPa,    NA_real_),
        dP_max_hPa    = dplyr::if_else(is.finite(.data$dP_max_hPa),    .data$dP_max_hPa,    NA_real_),
  ...
  #' Compute annual counts of unique storm events by severity
  #'
  #' @param events Tibble with at least columns: year, severity, SID.
  #' @param severities Character vector of severities to include.
  #'
  #' @return Tibble with columns year, severity, n_events, completed to include
  ...
  compute_annual_counts <- function(events, severities = c("TS", "HUR64plus")) {
    ...
    events |>
      dplyr::filter(.data$severity %in% severities) |>
      dplyr::distinct(.data$year, .data$severity, .data$SID) |>
      dplyr::count(.data$year, .data$severity, name = "n_events") |>
      tidyr::complete(
  ...
  #' Compute Poisson rate table by severity from annual counts
  #'
  #' @param annual_counts Output from \code{compute_annual_counts()}.
  #' @return Tibble with lambda, n_years, p_at_least_one, p_zero by severity.
  #' @export
  compute_lambda_table <- function(annual_counts) {
    ...
    annual_counts |>
      ...
      dplyr::summarise(
        ...
        n_years = dplyr::n(),
        p_at_least_one = 1 - exp(-.data$lambda),
        p_zero = exp(-.data$lambda),
        .groups = "drop"
      )
  }
  ...
  #' Var(N) = mu + mu^2/k to estimate k. If Var <= mu, returns a large k (≈Poisson).
  #'
  #' @param annual_counts Output from \code{compute_annual_counts()}.
